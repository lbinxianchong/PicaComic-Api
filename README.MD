...
如有任何建议欢迎 ISSUE 或 mail:[shirosaki@flannep.com][1]

V1.0版本已经移动到旧分支，不再进行更新。如有需求请切换分支。
master分支为最新版本。  


## 声明
> 本项目是个人的java学习项目，而不是一个成熟稳定的项目。随时可能会有巨大更改（甚至整个重写），目前没有向前兼容的打算，如果您要在生产环境使用此api请慎重考虑！  
